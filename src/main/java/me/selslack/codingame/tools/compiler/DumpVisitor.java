...
public class DumpVisitor implements VoidVisitor<Object> {
    ...
    private void printTypeArgs(final List<com.github.javaparser.ast.type.Type> args, final Object arg) {
        if (!isNullOrEmpty(args)) {
            printer.print("<");
            for (final Iterator<com.github.javaparser.ast.type.Type> i = args.iterator(); i.hasNext();) {
                final com.github.javaparser.ast.type.Type t = i.next();
                t.accept(this, arg);
                if (i.hasNext()) {
                    printer.print(", ");
                }
    ...
    @Override public void visit(MultiTypeParameter n, Object arg) {
        ...
        Iterator<com.github.javaparser.ast.type.Type> types = n.getTypes().iterator();
    ...
    @Override public void visit(final EnumConstantDeclaration n, final Object arg) {
        ...
        if (!isNullOrEmpty(n.getArgs())) {
            printArguments(n.getArgs(), arg);
        }

        if (!isNullOrEmpty(n.getClassBody())) {
            printer.printLn(" {");
            printer.indent();
